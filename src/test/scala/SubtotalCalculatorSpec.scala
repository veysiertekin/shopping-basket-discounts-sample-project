...
class SubtotalCalculatorSpec extends AnyFreeSpec with Matchers {
  "Subtotal calculation tests" - {
    ...
    "subtotal with discounted oranges in basket contains 3 apples & three orange" in {
      val basket = Basket().add("apple").add("apple").add("apple").add("orange").add("orange").add("orange")
      val calculator =
        SubtotalCalculator(basket, Map("orange" -> 0.25d, "apple" -> 0.6d), List(ThreeForTwoDiscount("orange")))
      calculator.total mustBe 2.30d
    }
  }
}
